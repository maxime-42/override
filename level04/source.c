...
int main(void) {
    ...
    // Create an array to hold shellcode
    char shellcode[32];
    ...
    // Initialize the shellcode array to zeros
    for (int i = 0; i < 32; i++) {
        shellcode[i] = 0;
    }
    ...
    if (childPid == 0) {
        ...
        ptrace(PTRACE_TRACEME, 0, 0, 0);  // Enable tracing on the child
        puts("Give me some shellcode, k");
        gets(shellcode);  // Get user input (shellcode)
    } else {
        // Parent process
      long userRegs  = 0;
        do {
            ...
            // Check if the child process is exiting
            if ((traceEvent & 0x7f) == 0 || ((traceEvent & 0x7f) + 1) >> 1 > 0) {
                puts("Child is exiting...");
                return 0;
            }

            // Check if the child process is trying to execute
            userRegs = ptrace(PTRACE_PEEKUSER, childPid, 0x2c, 0);
            if (userRegs == 0xb) {
                puts("No exec() for you");
                kill(childPid, 9);  // Terminate the child process
            }
        } while (userRegs != 0xb);
    }
